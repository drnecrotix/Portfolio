...
This page describes the minimum practical requirements for running the Portfolio application.
...
- Prisma ORM
- PostgreSQL
- NextAuth/Auth.js-style authenticated Admin access
...
Building Next.js usually needs substantially more memory than simply running an already-built application. Shared hosting with aggressive memory limits may fail during `npm run build` even when runtime traffic would be light.
...
On the N0C environment, use the repository's compatibility build when native SWC is unavailable:

```bash
npm run build:n0c
```

CI should also pass the Prisma migration checks and visual smoke tests.
...
Mobile layout should be checked separately because viewport behavior (`dvh`, browser bars and safe areas) can differ significantly from desktop.

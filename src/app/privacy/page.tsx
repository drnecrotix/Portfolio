        ...
        >
            ...
            <section>
                ...
                <ul className="mt-4 list-disc">
                    ...
                    <li><strong>Traffic analytics:</strong> country code, broad device class (desktop, mobile, tablet or unknown), hourly page-view totals, estimated visit totals and short-lived traffic session records containing the current public path. When the hosting/CDN already supplies a city header, a coarse city label may also be kept in that short-lived session record for live analytics. City is not inferred through an IP-to-city lookup and precise coordinates are not collected. Country is taken from infrastructure/proxy country headers where available. If those headers are missing, the raw client IP address may be stored in the short-lived traffic-session record and used only for an IP-to-country lookup.</li>
            ...
            <section>
                ...
                <div className="mt-5 space-y-6">
                    ...
                    <div>
                        ...
                        <p className="mt-2">Limited first-party A/B tests compare presentation and navigation variants so the site can evaluate usability and content discovery. Variants are randomly assigned and kept stable for the current browser session using a short-lived first-party cookie. The server stores aggregate experiment event totals rather than a named visitor profile. The legal basis relied on for this product-improvement processing is the legitimate interest in improving the website while minimising the data used (Article 6(1)(f)).</p>
                    </div>
                    <div>
                        <h3>Short-retention traffic analytics</h3>
                        <p className="mt-2">NecrotixLab measures broad traffic trends to understand visits, which public pages are active, which countries visit the site and what broad device classes are used. Country headers supplied by the hosting/CDN are preferred. If the infrastructure also supplies a city header, that coarse city label may be shown only with short-lived live-session analytics and is not copied into the longer-lived country/device aggregate table. When country headers are unavailable, the server may temporarily store the raw client IP address and request only a country code from the country.is IP-to-country service. No IP-to-city request or precise-coordinate collection is performed. The public page-view tracker ignores tracking when the browser sends the Do Not Track signal. The legal basis relied on for this limited measurement is the legitimate interest in maintaining and improving the site with short retention and data minimisation (Article 6(1)(f)).</p>
                    </div>
                    <div>
                        <h3>AI assistant</h3>
            ...
            <section>
                ...
                <p className="mt-3">Hosting or proxy infrastructure may provide country-level and, where supported, coarse city request headers used by the first-party traffic analytics layer. City headers remain in short-lived traffic-session data only. If no supported country header is available, the server may send the visitor IP address to <strong>country.is</strong> only to resolve an ISO country code. NecrotixLab stores the country code in aggregate traffic metrics; the raw IP remains only in the short-lived traffic-session record and is not included in the long-term aggregate table.</p>
            ...
            <section>
                ...
                <ul className="mt-4 list-disc">
                    ...
                    <li>Traffic-session records, including the current public path, an optional infrastructure-provided city label and a raw client IP address when available, are intended to be removed after about 24 hours of inactivity. A new analytics visit is counted after about 30 minutes of inactivity. Country/device traffic aggregates do not contain the raw IP or city label and are intended to be removed after about 31 days, supporting the 24-hour, 7-day and 30-day dashboard views without keeping long-term IP or city history.</li>
            ...
            <section>
                ...
                <p className="mt-4">NecrotixLab does not use the website data described here to make decisions about visitors that produce legal or similarly significant effects. A/B variants are randomly assigned for interface comparison and remain stable only for the current browser session. Traffic analytics is used for aggregate site measurement and short-lived live monitoring, not individual profiling. The AI assistant generates conversational responses but is not used for automated eligibility, employment, credit or similar decisions.</p>
            </section>

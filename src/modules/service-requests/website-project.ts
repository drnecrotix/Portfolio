...
export const websiteProjectLabels = {
    ...
    design: {
        starter: 'Starter / adapted design system',
        custom: 'Custom visual design',
        premium: 'Premium bespoke design',
    } satisfies Record<WebsiteDesignLevel, string>,
    cms: {
        none: 'No CMS required',
        wordpress: 'WordPress',
        headless: 'Headless CMS',
        custom: 'Custom administration',
    } satisfies Record<WebsiteCms, string>,
    content: {
        ready: 'Content is ready',
        assistance: 'Content needs assistance',
        full: 'Content preparation required',
    } satisfies Record<WebsiteContentSupport, string>,
    seo: {
        basic: 'Technical SEO basics',
        advanced: 'Advanced SEO setup',
    } satisfies Record<WebsiteSeoLevel, string>,
    hosting: {
        existing: 'Existing hosting',
        setup: 'Hosting setup required',
        managed: 'Managed hosting requested',
    } satisfies Record<WebsiteHosting, string>,
    deadline: {
        flexible: 'Flexible timeline',
        standard: 'Standard timeline',
        priority: 'Priority delivery',
    } satisfies Record<WebsiteDeadline, string>,
    maintenance: {
        none: 'No ongoing maintenance yet',
        monthly: 'Monthly maintenance requested',
    } satisfies Record<WebsiteMaintenance, string>,
    integration: {
        analytics: 'Analytics',
...
const projectBase: Record<WebsiteProjectType, { min: number; max: number; includedPages: number; weeks: [number, number] }> = {
...
const designCost: Record<WebsiteDesignLevel, [number, number]> = {
    starter: [0, 0],
    custom: [180, 320],
    premium: [420, 700],
};

const cmsCost: Record<WebsiteCms, [number, number]> = {
    none: [0, 0],
    wordpress: [90, 180],
    headless: [220, 420],
    custom: [420, 850],
...
export function parseWebsiteProjectScope(value: unknown): WebsiteProjectScope | null {
    ...
    if (!isOneOf(raw.deadline, deadlineOptions) || !isOneOf(raw.maintenance, maintenanceOptions)) return null;
...
export function estimateWebsiteProject(scope: WebsiteProjectScope) {
    const base = projectBase[scope.projectType];
    let min = base.min;
    let max = base.max;
    ...
    const [designMin, designMax] = designCost[scope.design];
    min += designMin;
    max += designMax;

    const [cmsMin, cmsMax] = cmsCost[scope.cms];
    min += cmsMin;
    max += cmsMax;

    const extraLanguages = Math.max(0, scope.languages - 1);
    min += extraLanguages * 110;
    max += extraLanguages * 190;
    ...
    for (const integration of scope.integrations) {
        const [integrationMin, integrationMax] = integrationCost[integration];
        min += integrationMin;
        max += integrationMax;
    }

    if (scope.deadline === 'flexible') {
        min *= 0.95;
        max *= 0.95;
    } else if (scope.deadline === 'priority') {
        min *= 1.2;
        max *= 1.28;
    }
    ...
    if (scope.deadline === 'priority') {
        weeksMin = Math.max(1, weeksMin - 1);
        weeksMax = Math.max(weeksMin, weeksMax - 1);
    }

    return {
        min: roundTen(min),
        max: Math.max(roundTen(min) + 50, roundTen(max)),
        currency: 'EUR' as const,
        weeks: { min: weeksMin, max: weeksMax },
        maintenanceQuotedSeparately: scope.maintenance === 'monthly',
    };
}

export function websiteProjectScopeLines(scope: WebsiteProjectScope): WebsiteProjectScopeLine[] {
    const integrations = scope.integrations.length
        ? scope.integrations.map((item) => websiteProjectLabels.integration[item]).join(', ')

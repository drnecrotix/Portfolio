...
const siteTypeIcons: Record<string, LucideIcon> = {
    ...
    'site-portfolio': UserRound,
    'site-business': Briefcase,
    'site-blog': Newspaper,
    'site-store': ShoppingCart,
    'site-community': Gamepad2,
    'site-recipes': Utensils,
    'site-knowledge': BookOpen,
    'site-courses': GraduationCap,
    'site-booking': CalendarDays,
    'site-directory': ListTree,
    'site-custom': PanelsTopLeft,
};
...
const goalBySiteType = Object.fromEntries(Object.entries(recommendations).map(([goal, recommendation]) => [recommendation.siteType, goal])) as Record<string, string>;
const wordpressPlugins: Record<string, string[]> = {
    'site-landing': ['Fluent Forms', 'Rank Math SEO', 'Site Kit by Google'],
    'site-portfolio': ['ACF', 'Fluent Forms', 'Rank Math SEO'],
    'site-business': ['Fluent Forms', 'Rank Math SEO', 'WP Mail SMTP'],
    'site-blog': ['Rank Math SEO', 'WP Rocket or LiteSpeed Cache', 'MailPoet'],
    'site-store': ['WooCommerce', 'Rank Math SEO', 'WooPayments or Stripe'],
    'site-community': ['BuddyPress or bbPress', 'Discord integration', 'Wordfence'],
    'site-recipes': ['WP Recipe Maker', 'FacetWP', 'Rank Math SEO'],
    'site-knowledge': ['BetterDocs', 'Relevanssi', 'Rank Math SEO'],
    'site-courses': ['Tutor LMS or LearnDash', 'Paid Memberships Pro', 'WooCommerce'],
    'site-booking': ['Amelia or Bookly', 'WP Mail SMTP', 'WooCommerce'],
    'site-directory': ['GeoDirectory', 'FacetWP', 'WP All Import'],
...
function existingRecommendation(siteType: string, platform: string): Recommendation {
    ...
    const byType: Record<string, FeatureId[]> = {
        ...
        'site-custom': ['feature-staging', 'feature-integrations'],
    };
    const cmsSpecific: FeatureId[] = ['Next.js', 'Strapi', 'Sanity', 'Directus', 'Custom CMS / development'].includes(platform)
        ? ['feature-staging', 'feature-integrations'] : [];
    return { title: `${platform === 'Recommend the best option' ? 'CMS-aware' : platform} redesign and expansion plan`, platform, reason: 'Focus on restructuring, redesigning or expanding the website. Repairs, security and ongoing maintenance belong in Website Support.', alternatives: [], features: [...new Set([...common, ...(byType[siteType] ?? []), ...cmsSpecific])] };
}
...
export function WebsiteOrderForm({ websiteBuildBase = {} }: { websiteBuildBase?: WebsiteBuildBase }) {
    ...
    useEffect(() => {
        if (step > 1) stepPanelRef.current?.focus();
    }, [step]);
    ...
    return <form onSubmit={submit} className="mt-10 grid gap-8 lg:grid-cols-[minmax(0,1fr)_300px]">
        <div>
            <nav aria-label="Website request progress" className="mb-10 grid grid-cols-4 border-y border-border/70">
                {['Project', 'Setup', 'Options', 'Contact'].map((label, index) => { const number = index + 1; return <button key={label} type="button" aria-current={number === step ? 'step' : undefined} onClick={() => number < step && setStep(number)} disabled={number > step} className={`border-r border-border/60 px-2 py-3 text-left last:border-r-0 ${number === step ? 'bg-sky-500/[0.07] text-foreground' : 'text-muted-foreground'}`}><span className="block font-mono text-[8px] font-bold text-sky-500">0{number}</span><span className="mt-1 block text-[10px] font-bold sm:text-xs">{label}</span></button>; })}
            </nav>

            <div ref={stepPanelRef} tabIndex={-1} className="outline-none">

            {step === 1 ? <div className="space-y-8">
                <Section number="01" title="What do you need?"><div className="grid border-y border-border/70 sm:grid-cols-3"><Card icon={Globe2} active={projectState === 'new'} title="A new website" detail="Plan a new website from the first page." onClick={() => selectProjectState('new')} /><Card icon={RefreshCw} active={projectState === 'existing'} title="Redesign or expand" detail="Change the structure, design or functionality." onClick={() => selectProjectState('existing')} /><ServiceLinkCard /></div></Section>
                <Section number="02" title={projectState === 'new' ? 'Choose the website type' : 'Tell us about the current website'}><div className="space-y-5">{projectState === 'existing' ? <label className="block text-xs text-muted-foreground">Website URL<input value={existingWebsite} onChange={(event) => { setExistingWebsite(event.target.value); setWebsiteUrlTouched(true); }} onBlur={() => setWebsiteUrlTouched(true)} type="url" inputMode="url" aria-invalid={websiteUrlTouched && !websiteUrlIsValid} aria-describedby="existing-website-error" placeholder="https://example.com" className="mt-2 w-full border border-border bg-transparent px-4 py-3 text-sm text-foreground outline-none transition focus:border-sky-500" />{websiteUrlTouched && !websiteUrlIsValid ? <span id="existing-website-error" role="alert" className="mt-2 block text-xs text-rose-500">Enter an address such as https://example.com</span> : null}</label> : null}<SiteTypePicker value={siteType} onChange={setSiteType} />{projectState === 'new' ? <label className="block text-xs text-muted-foreground">Audience (optional)<input value={audience} onChange={(event) => setAudience(event.target.value)} placeholder="Who is it for?" className="mt-2 w-full border-0 border-b border-border bg-transparent py-2 text-sm text-foreground outline-none focus:border-sky-500" /></label> : <div><p className="mb-3 text-xs text-muted-foreground">What is it built with?</p><CmsChoices value={platform} onChange={setPlatform} /></div>}</div></Section>
                <WizardNext onClick={() => { setWebsiteUrlTouched(true); if (projectStepIsComplete) setStep(2); }} disabled={!projectStepIsComplete} />
            </div> : null}
            ...
            {step === 4 ? <div className="space-y-8"><Section number="01" title="Project and contact details"><div className="grid gap-5 sm:grid-cols-2"><Field name="projectName" label="Project name" value={projectName} onChange={setProjectName} required /><Field name="name" label="Your name" value={customerName} onChange={setCustomerName} required /><Field name="email" label="Email" type="email" value={email} onChange={setEmail} required /><Field name="budget" label="Budget in EUR (optional)" type="number" value={budget} onChange={setBudget} /><TextArea name="requiredPages" label="What is most important?" value={requiredPages} onChange={setRequiredPages} placeholder="Key pages, functions or improvements..." /><TextArea name="message" label="Anything else? (optional)" value={additionalMessage} onChange={setAdditionalMessage} placeholder="Deadline, examples or special requirements..." rows={4} /></div><input name="company" className="hidden" tabIndex={-1} /><input name="competitorExamples" className="hidden" tabIndex={-1} /><input name="website" tabIndex={-1} autoComplete="off" className="hidden" aria-hidden="true" /><label className="mt-5 flex items-start gap-3 text-xs leading-5 text-muted-foreground"><input name="privacyAccepted" type="checkbox" checked={privacyAccepted} onChange={(event) => setPrivacyAccepted(event.target.checked)} required className="mt-0.5 size-4" />I agree that these details may be stored to process the request.</label></Section><div className="flex justify-start"><button type="button" onClick={() => setStep(3)} className="inline-flex items-center gap-2 px-1 py-2 text-xs font-bold text-muted-foreground hover:text-foreground"><ArrowLeft className="size-3.5" />Back</button></div></div> : null}
            </div>
        </div>
        <aside className="h-fit border-y border-border/80 py-5 lg:sticky lg:top-28"><p className="font-mono text-[9px] font-bold uppercase tracking-[0.18em] text-sky-500">Estimated range</p>{siteType ? <><p className="mt-2 text-4xl font-black tracking-[-0.05em]">€{estimate.min}-€{estimate.max}</p><p className="mt-3 text-xs leading-5 text-muted-foreground">Updated from your choices. Final price follows a manual review.</p><div className="mt-5 border-y border-border/60 py-3 text-xs"><p><strong>{projectState === 'existing' ? 'Website redesign or expansion' : siteTypes.find(([id]) => id === siteType)?.[1]}</strong></p><p className="mt-1 text-muted-foreground">{pageCount >= 15 ? '15+' : pageCount} {pageCount === 1 ? 'page' : 'pages'}</p><p className="mt-1 text-muted-foreground">{platform}</p><p className="mt-1 text-muted-foreground">{selectedFeatures.size} selected add-ons</p></div></> : <div className="mt-3 border-y border-border/60 py-4"><p className="text-sm font-bold">Choose a website type</p><p className="mt-1 text-xs leading-5 text-muted-foreground">Your estimate will appear after the first project choice.</p></div>}{step === 4 ? <button disabled={loading} className="mt-5 inline-flex w-full items-center justify-center gap-2 border border-foreground bg-foreground px-5 py-3 text-sm font-bold text-background transition hover:bg-transparent hover:text-foreground disabled:opacity-50">{loading ? <Loader2 className="size-4 animate-spin" /> : null}{loading ? 'Creating...' : 'Create request'}</button> : <button type="button" disabled={step === 1 && !projectStepIsComplete} onClick={() => { if (step === 1) setWebsiteUrlTouched(true); if (step > 1 || projectStepIsComplete) setStep(Math.min(4, step + 1)); }} className="mt-5 inline-flex w-full items-center justify-center gap-2 border border-foreground px-5 py-3 text-sm font-bold transition hover:bg-foreground hover:text-background disabled:cursor-not-allowed disabled:opacity-40">Continue <ArrowRight className="size-4" /></button>}{message ? <p role="status" className={`mt-4 text-xs leading-5 ${reference ? 'text-emerald-500' : 'text-muted-foreground'}`}>{message}</p> : null}{statusUrl ? <a href={statusUrl} className="mt-3 inline-flex items-center gap-2 text-xs font-bold text-sky-500 hover:underline">Open private status page <ExternalLink className="size-3" /></a> : null}</aside>
    </form>;
}
...
function Card({ icon: Icon, active, title, detail, onClick }: { icon: LucideIcon; active: boolean; title: string; detail: string; onClick: () => void }) { return <button type="button" aria-pressed={active} onClick={onClick} className={`min-h-28 border-b border-border/60 p-4 text-left transition sm:border-r ${active ? 'bg-sky-500/[0.07]' : 'hover:bg-foreground/[0.03]'}`}><span className="mb-3 inline-flex size-8 items-center justify-center border border-sky-500/30 text-sky-500"><Icon className="size-4" /></span><span className="flex items-center justify-between text-sm font-bold">{title}{active ? <Check className="size-4 text-sky-500" /> : null}</span><span className="mt-1 block text-xs leading-5 text-muted-foreground">{detail}</span></button>; }
function ServiceLinkCard() { return <Link href="/services/support" className="min-h-28 border-b border-border/60 p-4 text-left transition hover:bg-foreground/[0.03]"><span className="mb-3 inline-flex size-8 items-center justify-center border border-cyan-500/30 text-cyan-500"><Wrench className="size-4" /></span><span className="flex items-center justify-between text-sm font-bold">Fix or maintain<ArrowRight className="size-4 text-cyan-500" /></span><span className="mt-1 block text-xs leading-5 text-muted-foreground">Repairs, speed, security or ongoing support.</span></Link>; }
function SiteTypePicker({ value, onChange }: { value: string; onChange: (value: string) => void }) { return <fieldset><legend className="mb-3 text-xs text-muted-foreground">Website type</legend><div className="grid border-l border-t border-border/70 sm:grid-cols-2">{siteTypes.map(([id, label]) => { const Icon = siteTypeIcons[id] ?? FileText; const active = value === id; return <button key={id} type="button" aria-pressed={active} onClick={() => onChange(id)} className={`flex min-h-12 items-center gap-3 border-b border-r border-border/70 px-3 py-2 text-left text-xs transition ${active ? 'bg-sky-500/[0.08] font-bold text-foreground' : 'text-muted-foreground hover:bg-foreground/[0.03] hover:text-foreground'}`}><Icon className={`size-4 shrink-0 ${active ? 'text-sky-500' : ''}`} /><span className="flex-1">{label}</span>{active ? <Check className="size-3.5 text-sky-500" /> : null}</button>; })}</div></fieldset>; }
function PageSlider({ value, onChange }: { value: number; onChange: (value: number) => void }) { return <label className="block"><span className="flex items-end justify-between gap-4"><span className="text-xs text-muted-foreground">Approximate number of pages</span><strong className="text-2xl">{value >= 15 ? '15+' : value}</strong></span><input type="range" min="1" max="15" step="1" value={value} onChange={(event) => onChange(Number(event.target.value))} aria-valuetext={value >= 15 ? '15 or more pages' : `${value} ${value === 1 ? 'page' : 'pages'}`} className="mt-4 w-full accent-sky-500" /><span className="mt-2 flex justify-between font-mono text-[9px] text-muted-foreground"><span>1 - Home page</span><span>15+</span></span></label>; }
function PluginRecommendations({ plugins }: { plugins: string[] }) { return <div className="mt-5 border-t border-sky-500/20 pt-4"><p className="font-mono text-[8px] font-bold uppercase tracking-wider text-muted-foreground">Recommended WordPress tools</p><div className="mt-2 flex flex-wrap gap-2">{plugins.map((plugin) => <span key={plugin} className="border border-border bg-background px-2.5 py-1.5 text-[11px]">{plugin}</span>)}</div><p className="mt-2 text-[10px] leading-4 text-muted-foreground">Final plugins are confirmed after checking compatibility and licence costs.</p></div>; }
function FeatureGrid({ items, selected, onToggle, recommended = false }: { items: readonly (readonly [string, string])[]; selected: Set<string>; onToggle: (id: string) => void; recommended?: boolean }) { return <div className="grid border-l border-t border-border/70 sm:grid-cols-2">{items.map(([id, label]) => <label key={id} className={`flex min-h-12 cursor-pointer items-center gap-3 border-b border-r border-border/70 px-3 py-2 ${recommended ? 'text-sky-600 dark:text-sky-400' : ''}`}><input type="checkbox" checked={selected.has(id)} onChange={() => onToggle(id)} className="size-4" /><span className="text-xs">{label}{recommended ? <span className="ml-2 font-mono text-[7px] uppercase">suggested</span> : null}</span></label>)}</div>; }
...
function CmsOptionButtons({ value, onChange, options }: { value: string; onChange: (value: string) => void; options: readonly string[] }) { return <div className="flex flex-wrap gap-2">{options.map((option) => <button key={option} type="button" aria-pressed={value === option} onClick={() => onChange(option)} className={`border px-3 py-2 text-xs transition ${value === option ? 'border-sky-500 bg-sky-500 font-bold text-white' : 'border-border hover:border-foreground'}`}>{option}</button>)}</div>; }

...
If N0C provides a Node selector, configure the application to use the selected Node 22 runtime before installing packages.
...
If this command reports a missing environment variable, fix the Node application environment rather than hardcoding the secret in source files.
...
For a release that must be visible in Admin, increment the application version before merge, for example:

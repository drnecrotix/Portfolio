...
import { ArrowLeft, Camera, MapPin, Palette, Tag } from 'lucide-react';
...
export default async function GalleryWorkPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      ...
      <div className="mx-auto max-w-[1500px] px-5 pb-24 pt-28 sm:px-8 lg:px-12 lg:pt-32">
        ...
        <header className="mt-10 grid gap-8 border-b border-foreground/10 pb-10 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-end">
          <div>
            <div className="mb-5 flex flex-wrap gap-2">
              <span className="rounded-full border border-foreground/10 px-3 py-1 text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{creativeType}</span>
              {item.category && item.category !== creativeType && <span className="rounded-full border border-foreground/10 px-3 py-1 text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{item.category}</span>}
            </div>
            <h1 className="max-w-5xl font-serif text-4xl leading-[0.98] tracking-tight sm:text-6xl lg:text-7xl">{item.title}</h1>
            {item.description && <p className="mt-6 max-w-3xl text-base leading-8 text-muted-foreground sm:text-lg">{item.description}</p>}
          </div>
        ...
        <section className="mt-8 overflow-hidden rounded-[1.5rem] border border-foreground/10 bg-foreground/[0.02]">
          {item.type === 'video' ? (
        ...
        {images.length > 1 && (
          <section className="mt-12">
            <div className="mb-6 flex items-center gap-3">
              ...
              <h2 className="font-serif text-2xl sm:text-3xl">Series</h2>
            ...
            <div className="columns-1 gap-4 space-y-4 md:columns-2 xl:columns-3">
              {images.slice(1).map((image, index) => (
                <figure key={`${image}-${index}`} className="break-inside-avoid overflow-hidden rounded-2xl border border-foreground/10 bg-foreground/[0.02]">
                  <Image
                    src={image}
                    alt={`${item.altText || item.title} - ${index + 2}`}
                    width={1200}
                    height={900}
                    loading="lazy"
        ...
        <div className="mt-16 grid gap-12 lg:grid-cols-[minmax(0,1fr)_420px]">
          <div>
            {item.story && (
              <section>
                <div className="mb-5 flex items-center gap-3">
                  ...
                  <h2 className="font-serif text-3xl">About this work</h2>
                </div>
                <div className="max-w-3xl space-y-5 text-base leading-8 text-foreground/75">
            ...
            {item.tags.length > 0 && (
              <section className={item.story ? 'mt-12' : ''}>
          ...
          <aside className="h-fit rounded-2xl border border-foreground/10 bg-foreground/[0.02] p-5 sm:p-6">

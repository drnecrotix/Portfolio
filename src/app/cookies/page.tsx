    ...
    return (
        ...
        >
            ...
            <section>
                ...
                <p className="mt-3">Short-retention traffic analytics uses a random HTTP-only session cookie to estimate visits and live traffic. The server hashes the random cookie value before storing it. A visit is counted again after about 30 minutes of inactivity. Country headers supplied by the hosting/CDN are preferred; if the infrastructure also supplies a city header, that coarse city label can be kept only in the short-lived traffic-session record for live analytics. No IP-to-city lookup is performed. If no supported country header is available, the server may also store the raw client IP address in the same short-lived traffic-session record and use it only to resolve an ISO country code. Raw IP and city labels are not copied into the longer-lived aggregate traffic table. Traffic-session rows are intended to expire after about 24 hours of inactivity, while country/device aggregates are intended to expire after about 31 days.</p>

        ...
        <main className="min-h-screen bg-background pb-24 pt-28 text-foreground sm:pt-32">
            <header className="container mx-auto max-w-5xl px-6">
                <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.48, ease: 'easeOut' }} className="mx-auto max-w-3xl">
                    <button
                        onClick={goBack}
                        className="group mb-10 inline-flex items-center gap-2 text-sm text-muted-foreground transition hover:text-foreground"
                    >
                    ...
                    <h1 className="text-4xl font-black leading-[1.04] tracking-[-0.035em] sm:text-5xl lg:text-6xl">{title}</h1>
                    {excerpt && <p className="mt-7 max-w-2xl text-lg font-light leading-8 text-muted-foreground sm:text-xl">{excerpt}</p>}
            ...
            {featuredImage && (
                <div className="container mx-auto mt-12 max-w-5xl px-6">
                    <motion.figure
                        ...
                        className="overflow-hidden rounded-2xl border border-foreground/10 bg-foreground/[0.025]"
                    >
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        <img src={featuredImage} alt={title} className="max-h-[38rem] w-full object-cover" />
                    </motion.figure>
                </div>
            )}
                ...
                >
                    ...
                    <div className="flex flex-wrap items-center justify-end gap-2">
                        <motion.button
                            ...
                            whileTap={{ scale: 0.9 }}
                            className={`inline-flex h-9 items-center gap-2 rounded-full border px-3 text-xs font-medium transition ${liked ? 'border-rose-500/25 bg-rose-500/10 text-rose-500' : 'border-foreground/10 bg-foreground/[0.03] text-muted-foreground hover:text-foreground'}`}
                        >
                ...
                >
                    ...
                    {tags.length > 0 && (
                        <section className="mt-16 flex flex-wrap gap-x-4 gap-y-2 border-t border-foreground/10 pt-7">
                            {tags.map((tag) => (
                                <Link key={tag} href={`/blog?q=${encodeURIComponent(tag)}`} className="font-mono text-xs text-muted-foreground transition hover:text-foreground">
                                    #{tag.replace(/^#/, '')}
                                </Link>
                            ))}
                        </section>

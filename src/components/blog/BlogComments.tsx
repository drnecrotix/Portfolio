...
export function BlogComments({ postId, initialComments }: { postId: string; initialComments: PublicBlogComment[] }) {
    ...
    useEffect(() => {
        let active = true;
        fetchChallenge()
            .then((data) => { if (active) setChallenge(data); })
            .catch((challengeError) => { if (active) setError(challengeError instanceof Error ? challengeError.message : 'Unable to load bot check.'); })
            .finally(() => { if (active) setLoadingChallenge(false); });
        return () => { active = false; };
    }, []);
    ...
    return (
        <section className="mx-auto mt-14 w-full max-w-5xl border-t border-foreground/10 px-0 pb-10 pt-10 sm:mt-20 sm:pt-14" aria-labelledby="comments-title">
            ...
            {comments.length === 0 ? (
                <div className="mb-10 rounded-2xl border border-dashed border-foreground/10 px-5 py-8 text-center">
            ...
            <form id="comment-composer" onSubmit={submit} className="rounded-3xl border border-foreground/10 bg-foreground/[0.018] p-4 shadow-sm sm:p-6 lg:p-8">
                <div className="mb-6 flex items-start justify-between gap-4 border-b border-foreground/10 pb-5">
                    ...
                    {replyTarget && (
                        <button type="button" onClick={() => setReplyTarget(null)} className="inline-flex size-9 shrink-0 items-center justify-center rounded-full border border-foreground/10 text-muted-foreground transition hover:bg-foreground/[0.06] hover:text-foreground" aria-label="Cancel reply" title="Cancel reply">
                            <X className="h-4 w-4" />
                        </button>
                    )}
                </div>
                ...
                <div className="mt-5 grid gap-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
                    <div className="rounded-2xl border border-foreground/10 bg-background/70 p-4 sm:p-5">
                        <div className="flex flex-col gap-4 sm:flex-row sm:items-end">
                            ...
                            <button type="button" onClick={() => void loadChallenge()} disabled={loadingChallenge} className="flex size-10 shrink-0 items-center justify-center rounded-xl border border-foreground/10 text-muted-foreground transition hover:bg-foreground/[0.05] hover:text-foreground disabled:opacity-40" aria-label="New bot-check question" title="New bot-check question">

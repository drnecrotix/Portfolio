...
export function WebsiteOrderForm() {
    ...
    const [domainResult, setDomainResult] = useState<{ status: string; message: string; domain?: string } | null>(null);
    ...
    const requirements = useMemo(() => {
        ...
        return rows;
    }, [brandReady, contentReady, design, goal, pages, platform, priority, selectedFeatures, siteType, style]);
    ...
    async function checkDomain() {
        ...
        try {
            ...
            const payload = await response.json().catch(() => ({})) as { status?: string; message?: string; domain?: string; error?: string };
            if (!response.ok) throw new Error(payload.error || 'Domain check failed.');
            setDomainResult({ status: payload.status || 'unknown', message: payload.message || 'No conclusive result.', domain: payload.domain });
    ...
    async function submit(event: FormEvent<HTMLFormElement>) {
        ...
        try {
            ...
            const response = await fetch('/api/service-requests', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({
                ...
                snapshot: { projectType: siteType, pages, design, features: [...selectedFeatures], priority, platform, goal, style, contentReady, brandReady, audience: form.get('audience'), competitorExamples: form.get('competitorExamples'), requiredPages: form.get('requiredPages'), existingWebsite: form.get('existingWebsite'), domainCheck: domainResult },

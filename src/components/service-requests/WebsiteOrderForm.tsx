    ...
    return <form onSubmit={submit} className="mt-10 grid gap-8 lg:grid-cols-[minmax(0,1fr)_300px]">
        <div>
            ...
            {step === 3 ? <div className="space-y-8">
                <Section number="01" title="Choose the essentials"><div className="space-y-6"><div><p className="mb-3 text-xs text-muted-foreground">Website type</p><SiteTypeChoices value={siteType} onChange={setSiteType} /></div><div><p className="mb-3 text-xs text-muted-foreground">Approximate size</p><ChoiceButtons value={pages} onChange={setPages} options={pageOptions} /></div><div className="border-l-2 border-sky-500 pl-3 text-xs"><span className="text-muted-foreground">Selected platform:</span> <strong>{platform}</strong>{projectState === 'new' ? <button type="button" onClick={() => setStep(2)} className="ml-3 text-sky-500 hover:underline">Change</button> : null}</div></div></Section>

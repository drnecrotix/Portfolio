...
import { z } from 'zod';
import { domainTld, normalizeDomainCandidate } from '@/modules/domain-availability/domain';
...
export async function POST(request: Request) {
    ...
    try {
        ...
        if (response.ok) return NextResponse.json({ domain, status: 'registered', message: 'A registration record exists for this domain.' }, { headers: noStoreHeaders });

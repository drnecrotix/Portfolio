...
export function ProjectForm({ project, action, submitLabel }: { project?: PrismaProject | null; action: (formData: FormData) => void | Promise<void>; submitLabel: string }) {
    const content = project?.content ? JSON.stringify(project.content, null, 2) : '{}';
    ...
    return (
        <form action={action} className="space-y-8">
            ...
            <section className="grid gap-5 rounded-2xl border border-white/10 bg-white/[0.02] p-6 md:grid-cols-2">
                ...
                <label className="block md:col-span-2"><span className="text-sm text-white/55">Advanced content JSON</span><p className="mt-1 text-xs text-white/30">Optional keys: image, galleryImages, features, installation, challengesAndSolutions.</p><textarea className={`${field} font-mono`} name="content" rows={16} defaultValue={content} /></label>
            </section>

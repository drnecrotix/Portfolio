...
function oneOffPrice(item: { priceFrom: number; priceTo: number | null }) {
...
export default async function ServicePricingPage() {
    ...
    const overrides = normalizeServicePricing(config?.content);
    const buildBase = overrides.websiteBuildBase;
    ...
    const oneOff = SERVICE_PRICING.oneOff.map((item) => ({ ...item, priceFrom: overrides.oneOff[item.id].min, priceTo: overrides.oneOff[item.id].max }));
    const monthly = SERVICE_PRICING.monthly.map((item) => ({ ...item, price: overrides.monthly[item.id] }));
    const supportOneOff = SERVICE_PRICING.supportOneOff.map((item) => { const range = overrides.supportOneOff[item.id]; return { ...item, price: range.min === range.max ? `€${range.min}` : `€${range.min}-€${range.max}` }; });
    const supportMonthly = SERVICE_PRICING.supportMonthly.map((item) => ({ ...item, price: overrides.supportMonthly[item.id] }));
    return (

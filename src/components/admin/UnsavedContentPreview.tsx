...
export function UnsavedContentPreview({ kind }: { kind: 'blog' | 'project' }) {
  ...
  const openPreview = (button: HTMLButtonElement) => {
    ...
    const rawContent = String(data.get(kind === 'blog' ? 'content' : 'longDescription') || '');
    const content = kind === 'blog' ? rawContent : rawContent;
  ...
  return (
    <>
      ...
      {html && (
        <div className="fixed inset-0 z-[200] bg-black/80 p-3 backdrop-blur-md sm:p-6">

...
/** Soft public URL probes for social platforms. False positives are possible when CDNs return generic pages. */
        ...
        const checks = unique.flatMap((username) => services.map(async (service) => {
            try {
                ...
                return finding({
                    ...
                    exposedData,
                    remediation: ['Open the profile and review privacy settings.', 'If this is not your account, treat the match as a username collision.'],
                });
            } catch {
                return null;
            }

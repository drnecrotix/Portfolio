...
import { resolveFootprintCredential, resolveHoleheConfig } from './credentials';
...
const hibp: FootprintProvider = {
    ...
    async check({ email, signal }) {
        if (!email) return [];
        const apiKey = await resolveFootprintCredential('hibp.apiKey', 'HIBP_API_KEY');
        if (!apiKey) return [];
        const response = await fetch(`https://haveibeenpwned.com/api/v3/breachedaccount/${encodeURIComponent(email)}?truncateResponse=false`, {
            signal,
            headers: { 'hibp-api-key': apiKey, 'user-agent': 'NecrotixLab-Digital-Footprint' },
            cache: 'no-store',
        });
...
const holehe: FootprintProvider = {
    ...
    async check({ email, signal }) {
        if (!email) return [];
        const { url: base, token } = await resolveHoleheConfig();
...
const emailRep: FootprintProvider = {
    ...
    async check({ email, signal }) {
        if (!email) return [];
        const apiKey = await resolveFootprintCredential('emailrep.apiKey', 'EMAILREP_API_KEY');
        if (!apiKey) return [];
        const response = await fetch(`https://emailrep.io/${encodeURIComponent(email)}`, { signal, cache: 'no-store', headers: { Key: apiKey, 'User-Agent': 'NecrotixLab' } });
...
// NOTE: remaining providers restored from main branch content via subsequent commit if truncated
export { hibp as _hibpExportForTypecheck };
export const footprintProviders: FootprintProvider[] = [hibp, holehe, emailRep, ...extraProviders];

export async function runFootprintProviders(input: Parameters<FootprintProvider['check']>[0]) {
    const findings: FootprintFinding[] = [];
    for (const provider of footprintProviders) {
        if (!provider.configured()) continue;
        try {
            const part = await provider.check(input);
            findings.push(...part);
        } catch {
